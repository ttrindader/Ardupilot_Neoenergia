#pragma once

//#include <SITL/SITL.h>

#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
#include "AP_InertialSensor.h"
#include "AP_InertialSensor_Backend.h"
#include <GCS_MAVLink/GazeboMsgs.h>


// simulated sensor rates in Hz. This matches a pixhawk1
const uint16_t INS_GAZEBO_SENSOR_A[] = { 1000, 1000 };
const uint16_t INS_GAZEBO_SENSOR_B[] = { 760, 800 };

class AP_InertialSensor_GAZEBO : public AP_InertialSensor_Backend
{
public:
    AP_InertialSensor_GAZEBO(AP_InertialSensor &imu, const uint16_t sample_rates[]);

    /* update accel and gyro state */
    bool update() override;
    void start() override;

    // detect the sensor
    static AP_InertialSensor_Backend *detect(AP_InertialSensor &imu, const uint16_t sample_rates[]);

private:
    //SITL::SITL *sitl;
    const uint16_t *sitl;
    bool init_sensor(void);
    void timer_update();
    void generate_accel();
    void generate_gyro();

    const uint16_t gyro_sample_hz;
    const uint16_t accel_sample_hz;

    uint8_t gyro_instance;
    uint8_t accel_instance;
    uint64_t next_gyro_sample;
    uint64_t next_accel_sample;
    
    uint8_t gyro_fail_mask;
    uint8_t accel_fail_mask;    

    static uint8_t bus_id;
};
#endif // CONFIG_HAL_BOARD
