...
const AP_Param::GroupInfo AC_PosControlRiver::var_info[] = {
    ...
    // @Increment: 0.1
    // @User: Advanced
    AP_GROUPINFO("_ACC_XY_FILT", 1, AC_PosControlRiver, _accel_xy_filt_hz, POSCONTROL_ACCEL_FILTER_HZ),
    ...
    // @Range: 1.000 3.000
    // @User: Standard
    AP_SUBGROUPINFO(_p_pos_z, "_POSZ_", 2, AC_PosControlRiver, AC_P),
    ...
    // @Range: 1.000 8.000
    // @User: Standard
    AP_SUBGROUPINFO(_p_vel_z, "_VELZ_", 3, AC_PosControlRiver, AC_P),
    ...
    // @Units: Hz
    // @User: Standard
    AP_SUBGROUPINFO(_pid_accel_z, "_ACCZ_", 4, AC_PosControlRiver, AC_PID),
    ...
    // @Range: 0.500 2.000
    // @User: Standard
    AP_SUBGROUPINFO(_p_pos_xy, "_POSXY_", 5, AC_PosControlRiver, AC_P),
    ...
    // @Units: Hz
    // @User: Advanced
    AP_SUBGROUPINFO(_pid_vel_xy, "_VELXY_", 6, AC_PosControlRiver, AC_PID_2D),
    ...
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("_ANGLE_MAX", 7, AC_PosControlRiver, _lean_angle_max, 0.0f),
...
// their values.
//
AC_PosControlRiver::AC_PosControlRiver(AP_AHRS_View& ahrs, const AP_InertialNav& inav,
                             const AP_Motors& motors, AC_AttitudeControl_River& attitude_control) :
    _ahrs(ahrs),
    _inav(inav),
    _motors(motors),
...
// void AC_PosControlRiver::calc_leash_length_z()
// {
...
void AC_PosControlRiver::set_max_accel_xy(float accel_cmss)
{
...
void AC_PosControlRiver::set_max_speed_xy(float speed_cms)
{
...
void AC_PosControlRiver::set_pos_target(const Vector3f& position)
{
...
void AC_PosControlRiver::set_xy_target(float x, float y)
{
    _pos_target.x = x;
    _pos_target.y = y;
}
...
void AC_PosControlRiver::shift_pos_xy_target(float x_cm, float y_cm)
{
...
void AC_PosControlRiver::set_target_to_stopping_point_xy()
{
...
void AC_PosControlRiver::get_stopping_point_xy(Vector3f &stopping_point) const
{
    const Vector3f curr_pos = _inav.get_position();
...
float AC_PosControlRiver::get_distance_to_target() const
{
    return norm(_pos_error.x, _pos_error.y);
}

/// get_bearing_to_target - get bearing to target position in centi-degrees
int32_t AC_PosControlRiver::get_bearing_to_target() const
{
...
void AC_PosControlRiver::relax_velocity_controller_xy()
{
...
bool AC_PosControlRiver::is_active_xy() const
{
    return ((AP_HAL::micros64() - _last_update_xy_us) <= POSCONTROL_ACTIVE_TIMEOUT_US);
}

/// get_lean_angle_max_cd - returns the maximum lean angle the autopilot may request
float AC_PosControlRiver::get_lean_angle_max_cd() const
{
...
void AC_PosControlRiver::init_xy_controller()
{
...
void AC_PosControlRiver::standby_xyz_reset()
{
...
void AC_PosControlRiver::update_xy_controller()
{
    // compute dt
    const uint64_t now_us = AP_HAL::micros64();
...
float AC_PosControlRiver::time_since_last_xy_update() const
{
    const uint64_t now_us = AP_HAL::micros64();
...
void AC_PosControlRiver::write_log()
{
...
void AC_PosControlRiver::init_vel_controller_xyz()
{
    // set roll, pitch lean angle targets to current attitude
    _roll_target = _ahrs.roll_sensor;
    _pitch_target = _ahrs.pitch_sensor;
...
void AC_PosControlRiver::update_vel_controller_xyz()
{
...
float AC_PosControlRiver::get_horizontal_error() const
{
    return norm(_pos_error.x, _pos_error.y);
}
...
void AC_PosControlRiver::calc_leash_length_xy()
{
...
void AC_PosControlRiver::desired_accel_to_vel(float nav_dt)
{
    // range check nav_dt
    if (nav_dt < 0) {
        return;
...
void AC_PosControlRiver::desired_vel_to_pos(float nav_dt)
{
    // range check nav_dt
    if (nav_dt < 0) {
        return;
...
void AC_PosControlRiver::run_xy_controller(float dt)
{
...
void AC_PosControlRiver::accel_to_lean_angles(float accel_x_cmss, float accel_y_cmss, float& roll_target, float& pitch_target) const
{
...
// get_lean_angles_to_accel - convert roll, pitch lean angles to lat/lon frame accelerations in cm/s/s
void AC_PosControlRiver::lean_angles_to_accel(float& accel_x_cmss, float& accel_y_cmss) const
{
...
float AC_PosControlRiver::calc_leash_length(float speed_cms, float accel_cms, float kP) const
{
    float leash_length;
...
void AC_PosControlRiver::init_ekf_xy_reset()
{
    Vector2f pos_shift;
...
void AC_PosControlRiver::check_for_ekf_xy_reset()
{
...
void AC_PosControlRiver::init_ekf_z_reset()
{
    float alt_shift;
...
/// check for ekf position reset and adjust loiter or brake target position
void AC_PosControlRiver::check_for_ekf_z_reset()
{
    // check for position shift
    float alt_shift;
...
bool AC_PosControlRiver::limit_vector_length(float& vector_x, float& vector_y, float max_length)
{
...
Vector3f AC_PosControlRiver::sqrt_controller(const Vector3f& error, float p, float second_ord_lim)
{
...
bool AC_PosControlRiver::pre_arm_checks(const char *param_prefix,
                                   char *failure_msg,
